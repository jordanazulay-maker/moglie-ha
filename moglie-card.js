class MoglieHaCard extends HTMLElement {
  ...
  set hass(hass) {
    ...
    const isArmed = alarmState.toLowerCase().includes('armed');
    ...
    if (!this.content) {
      this.innerHTML = `
        <ha-card>
          <style>
            ...
            .text-box { line-height: 1.5; margin-bottom: 10px; font-size: 1.1em; }
            .img-container img { width: 110px; transition: all 0.5s ease; }
            .debug-text { font-size: 0.7em; color: gray; margin-top: 10px; border-top: 1px solid #eee; padding-top: 5px; }
      ...
      this.debug = this.querySelector(".debug-text");
    }

    // Update the debug line so you can see what's happening
    this.debug.innerHTML = `WAN: ${wanState} | Alarm: ${alarmState}`;
